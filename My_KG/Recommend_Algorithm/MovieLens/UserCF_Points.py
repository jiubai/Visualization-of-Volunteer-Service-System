...
import random
import codecs
from Recommend_Algorithm.MovieLens.Distance import Cosin
import datetime
import math
...
def UserSimilarity_Cos(train):
    """
    生成用户相似度矩阵（基于修正的余弦距离，即用户评分预测的协同过滤）
    """
    ...
    for u in train:
        ...
        for v in train:
            if u == v:
                userSim[u][v] = -1
                continue
            userSim[u][v] = Cosin(train[u], train[v])
...
def testRecommend():
    ...
    userSim = UserSimilarity_Cos(train)
    user = "344"
