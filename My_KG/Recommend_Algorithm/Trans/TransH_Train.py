...
# 初始化
    def __init__(self, data_dir, negative_sampling,learning_rate,
        ...
        with tf.device('/cpu'):
            ...
            self.__normal_vector = tf.get_variable(name="normal_vector", shape=[self.__num_relation, self.__dimension],
                                                 initializer=tf.contrib.layers.xavier_initializer(uniform=False))
            self.__variables.append(self.__embedding_entity)
            self.__variables.append(self.__embedding_relation)
            self.__variables.append(self.__normal_vector)
...
# 载入数据
    def load_data(self):
        ...
        with open(os.path.join(self.__data_dir,'relation2id.txt'),'r',encoding='utf-8') as f:
        ...
        def load_triple(self, triplefile):
            ...
            with open(os.path.join(self.__data_dir, triplefile),'r',encoding='utf-8') as f:
                for line in f.readlines():
                    line_list = line.strip().split(',')
                    assert len(line_list) == 3  # 有不规则数据则中断
                    headid = self.__entity2id[line_list[0].encode('utf-8').decode('utf-8-sig').strip()]
                    relationid = self.__relation2id[line_list[1]]
                    tailid = self.__entity2id[line_list[2].encode('utf-8').decode('utf-8-sig').strip()]
        ...
        self.__triple = np.concatenate([self.__triple_train, self.__triple_test], axis = 0 )
...
#训练
    def train(self, inputs):
        ...
        normal_vector = self.__normal_vector
        ...
        pos_norm = tf.nn.embedding_lookup(norm_normal_vector, triple_positive[:, 1])
        neg_norm = tf.nn.embedding_lookup(norm_normal_vector, triple_negative[:, 1])

        pos_h_e = self.calc(embedding_positive_head, pos_norm)
        pos_t_e = self.calc(embedding_positive_tail, pos_norm)
        neg_h_e = self.calc(embedding_negative_head, neg_norm)
        neg_t_e = self.calc(embedding_negative_tail, neg_norm)

        score_positive = tf.reduce_sum(tf.abs(pos_h_e + embedding_positive_relation - pos_t_e), axis=1)
        score_negative = tf.reduce_sum(tf.abs(neg_h_e + embedding_negative_relation - neg_t_e), axis=1)
        ...
        return loss_triple, loss_every, norm_entity_l2sum
...
def main():
    parser = argparse.ArgumentParser(description="TransH")
    ...
    parser.add_argument('--max_iter', dest='max_iter', type=int, help='maximum interation', default=500)
    ...
    parser.add_argument('--n_test', dest='n_test', type=int, help='number of triples for test during the training', default=500)
    args = parser.parse_args()
    print(args)
    # 生成模型
    model = TransH(negative_sampling=args.negative_sampling, data_dir=args.data_dir,
    ...
    with tf.Session() as session:
        ...
        # 计算指标：
            if n_iter % args.evaluate_per_iteration == 0 or n_iter == 0 or n_iter == args.max_iter-1:   # 第一次迭代、到次数迭代、最后一次迭代
                ...
                for i in range(n_test):
                    ...
                    for i in range(len(id_replace_head)):
                        val = id_replace_head[-i-1]     # 倒着数最近的
                        if val == t[0]:
                            break
                        else:
                            hrank += 1
                            fhrank += 1
                            if val in tr_h[(t[2], t[1])]:   # 可能有多个头通过这个关系到尾
                                fhrank -= 1
        ...
        saver.save(session, './model/transh')
